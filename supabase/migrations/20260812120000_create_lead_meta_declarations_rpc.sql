-- META-ADS-003: return only the authorized declarations from one selected
-- materialized Meta Lead Ads event without exposing the ingestion inbox.

do $$
begin
  if to_regclass('public.profiles') is null
    or to_regclass('public.crm_leads') is null
    or to_regclass('public.lead_ingestion_events') is null
    or to_regprocedure('public.get_lead_monthly_investment_capacity(uuid)') is null then
    raise exception 'META declaration projection prerequisites are missing';
  end if;
  ...
  if not exists (
    ...
    raise exception 'lead_ingestion_events.form_id text is required';
  end if;
end
$$;
...
begin
  if v_function_oid is null then
    raise exception 'META declaration projection RPC is missing';
  end if;
  ...
  if (
    ...
    where namespace.nspname = 'public'
      and procedure.proname = 'get_lead_meta_declarations'
  ) <> 1 then
    raise exception 'Unexpected META declaration projection overload exists';
  end if;
  ...
  if not exists (
    ...
    where procedure.oid = v_function_oid
      ...
      and pg_get_function_result(procedure.oid) =
        'TABLE(monthly_investment_capacity text, declared_brazilian_and_cpf_status text)'
  ) then
    raise exception 'META declaration projection function contract is invalid';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'META declaration projection execution privileges are invalid';
  end if;

  if has_table_privilege(
    'authenticated',
    'public.lead_ingestion_events',
    'SELECT'
  ) then
    raise exception 'authenticated must not read the ingestion inbox directly';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception 'META-ADS-002 RPC compatibility contract changed';
  end if;
end
$$;
